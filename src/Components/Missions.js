        ...
        <tbody>
          ...
          {missions && missions.map((mission) => (
            <tr key={mission.mission_id}>
              ...
              <td className={`userBadge ${mission.reserved ? 'member' : ' '}`}>
                <label htmlFor="test">
                  {mission.reserved ? 'Active member' : 'NOT A MEMBER'}
                </label>
              </td>
              <td className="joinButton">
                <button type="button" onClick={() => dispatch(joinMission(mission.mission_id))} className={`missionBtn ${mission.reserved ? 'joined' : ' '}`}>
